    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl border border-white/10 bg-white/2 backdrop-blur-sm">
            ...
            <div class="grid grid-cols-1 divide-y divide-white/10 md:hidden">
                @forelse ($users as $user)
                    <div class="p-4 hover:bg-white/2 sm:p-5">
                        <div class="mb-5 flex min-w-0 items-center gap-4">
                            ...
                            <div class="flex min-w-0 flex-col">
                                <span class="truncate text-base font-bold text-white">{{ $user->name }}</span>
                        ...
                        <div class="space-y-4 text-sm">
                            <div class="flex min-w-0 flex-col gap-1">
                                ...
                                <span class="break-words text-gray-300">{{ $user->email }}</span>
                            </div>

                            <div class="grid grid-cols-2 gap-3">
                                <div class="flex min-w-0 flex-col gap-1">
                                    <span class="text-[10px] uppercase tracking-tighter text-gray-500">Phone</span>
                                    <span class="truncate text-gray-300">{{ $user->phone_number ?? 'N/A' }}</span>
                                </div>
                        ...
                        <div class="mt-6 flex items-center justify-end gap-2 border-t border-white/10 pt-4">
                            <a href="{{ route('admin.users.edit', $user->id) }}"
                               class="inline-flex min-h-10 items-center rounded-lg bg-indigo-600 px-4 text-sm font-semibold text-white transition-colors hover:bg-indigo-500">
                                Edit
                            </a>
                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                  onsubmit="return confirm('Are you sure you want to delete this user?');">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-danger flex min-h-10 min-w-10 items-center justify-center rounded-lg border-none bg-red-700 p-2 transition-colors hover:bg-red-600" type="submit" title="Delete user">
