    ...
    <main class="max-w-6xl mx-auto px-4 py-8 md:py-12">
            ...
            <div class="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            <span class="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full border
                                {{ $user->isStudent() ? 'text-blue-400 bg-blue-400/10 border-blue-400/20' : '' }}
                                {{ $user->isTeacher() ? 'text-cyan-400 bg-cyan-400/10 border-cyan-400/20' : '' }}
                                {{ $user->isEmployer() ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' : '' }}
                                {{ $user->isAdmin() ? 'text-purple-400 bg-purple-400/10 border-purple-400/20' : '' }}">
                                {{ ucfirst($user->role) }}
                            </span>
                            @if($user->employer)
                                <span class="inline-flex items-center gap-1.5 text-xs text-cyan-400 bg-cyan-400/10 px-3 py-1 rounded-full border border-cyan-400/20">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    @elseif($user->isStudent())
                        <a href="{{ route('home') }}" class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 text-sm md:text-base whitespace-nowrap">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif($user->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-blue-500 hover:from-purple-700 hover:to-blue-600 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 shadow-lg shadow-purple-500/20 hover:shadow-purple-500/40 text-sm md:text-base whitespace-nowrap">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @endif
                    <a href="{{ route('profile.edit') }}" class="text-center text-xs text-white/40 hover:text-white transition underline">
                        ...
                        <div class="bg-white/5 border border-white/10 p-4 rounded-xl flex justify-between items-center">
                            <div>
                                ...
                                @if(isset($notification->data['rejection_reason']))
                                    <p class="text-red-400 text-sm mt-1">Reason: {{ $notification->data['rejection_reason'] }}</p>
                                @endif
                                <p class="text-white/40 text-xs mt-1">{{ $notification->created_at->diffForHumans() }}</p>
                            </div>
                            <form action="{{ route('notifications.read', $notification->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="text-cyan-400 hover:text-cyan-300 text-sm font-medium">
                                    Mark as read
                                </button>
                            </form>
                        </div>
        ...
        @if($user->isStudent())
            ...
            <div class="flex items-center justify-between gap-4 border-b border-white/10 pb-4 mb-6">
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-white tracking-wide">Your Saved Jobs</h2>
                    <p class="text-white/40 text-sm mt-0.5">Jobs you've bookmarked for later</p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2">
                @forelse ($bookmarks as $job)
                    <x-job-card-wide :$job />
                @empty
                    <div class="md:col-span-2 border-2 border-dashed border-white/10 rounded-2xl p-12 text-center">
                        <div class="max-w-sm mx-auto">
                            <h3 class="text-white/60 font-medium mb-2">No saved jobs yet</h3>
                            <p class="text-white/40 text-sm mb-6">Bookmark jobs you're interested in to see them here.</p>
                            <a href="{{ route('home') }}" class="inline-block bg-white/10 hover:bg-white/20 text-white text-sm font-semibold px-6 py-3 rounded-lg transition-all duration-200">
            ...
            @if(count($bookmarks) >= 5)
                <div class="mt-8 text-center">
                    <a href="{{ route('bookmarks.index') }}" class="text-cyan-400 hover:underline text-sm font-medium">View all bookmarks</a>
                </div>
            @endif
        @endif

        @if($user->isTeacherOrEmployer())
            <!-- Jobs Header for Teachers/Employers -->
            <div class="flex items-center justify-between gap-4 border-b border-white/10 pb-4 mb-6">
                <div>
                        ...
                        <div class="flex flex-col h-full">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                                ...
                                <div class="flex items-center gap-3">
                                    <form action="{{ route('jobs.destroy', $job->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this job posting?');">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <a href="{{ route('jobs.edit', $job->id) }}" class="text-white/40 hover:text-cyan-400 transition-colors duration-200 text-sm font-medium">
                                        Edit
                                    </a>
                                </div>
                            </div>
                ...
                @empty
                    <div class="md:col-span-2 border-2 border-dashed border-white/10 rounded-2xl p-12 text-center">
                        <div class="max-w-sm mx-auto">
                            ...
                            <a href="{{ route('jobs.create') }}" class="inline-block bg-white/10 hover:bg-white/20 text-white text-sm font-semibold px-6 py-3 rounded-lg transition-all duration-200">
                                Post Your First Job
                            </a>
                        </div>
                    </div>
